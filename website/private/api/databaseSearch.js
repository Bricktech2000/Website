...
export default async function databaseSearch({ opr, tags, max, excl }) {
  ...
  for (var id of ids) {
    ...
    var infoText = await (await fetch('/' + id + '/index.json')).text();
    var indexText = await (await fetch('/' + id + '/index.md')).text();
